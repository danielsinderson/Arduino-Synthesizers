...
const byte ON_OFF_1 = 11; //voice 1 toggle
const byte ON_OFF_2 = 12; //voice 2 toggle
const byte ASSIGN_SETTINGS = 2;
const byte CHANNEL_SELECT = 21;
const byte CHANNEL_TOGGLE = 20;
...
int ch1[6] = {0, 0, 0, 0, 0, 0};
int ch2[6] = {0, 0, 0, 0, 0, 0};
...
void updateControl(){
  ...
  //Read analog inputs for oscillator objects if channel is on, otehrwise, set all to OFF values.
  ...
  //calculate values for updateAudio function for each channel
  ...
  //saved channel settings inputs
  if (digitalRead(ASSIGN_SETTINGS) == HIGH) {
    if (digitalRead(CHANNEL_SELECT) == LOW) {
      ch1[0] = carrier_freq_1;
      ch1[1] = mod_freq_1;
      ch1[2] = mod_speed_1;
      ch1[3] = carrier_freq_2;
      ch1[4] = mod_freq_2;
      ch1[5] = mod_speed_2;
    }
    else if (digitalRead(CHANNEL_SELECT) == HIGH) {
      ch2[0] = carrier_freq_1;
      ch2[1] = mod_freq_1;
      ch2[2] = mod_speed_1;
      ch2[3] = carrier_freq_2;
      ch2[4] = mod_freq_2;
      ch2[5] = mod_speed_2;
    }
  }
  ...
  //assign oscillator object frequencies
  if (digitalRead(CHANNEL_TOGGLE) == LOW) {
  ...
  else if (digitalRead(CHANNEL_TOGGLE) == HIGH) {
    if (digitalRead(CHANNEL_SELECT) == LOW) {
      aCarrier_1.setFreq(ch1[0]); 
      aModulator_1.setFreq(ch1[1]);
      kIntensityMod_1.setFreq(ch1[2]);
      aCarrier_2.setFreq(ch1[3]); 
      aModulator_2.setFreq(ch1[4]);
      kIntensityMod_2.setFreq(ch1[5]);
    }
    if (digitalRead(CHANNEL_SELECT) == HIGH) {
      aCarrier_1.setFreq(ch2[0]); 
      aModulator_1.setFreq(ch2[1]);
      kIntensityMod_1.setFreq(ch2[2]);
      aCarrier_2.setFreq(ch2[3]); 
      aModulator_2.setFreq(ch2[4]);
      kIntensityMod_2.setFreq(ch2[5]);
    }
  }
  /*
